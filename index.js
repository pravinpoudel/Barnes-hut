...
//not working right now; implement something with tree traversal;
function initializeNodes(n) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        let node = new Node(i, pos.x, pos.y);
        nodes.push(node);
        nodes2.push(node);
    }
}
...
function initializeNodesRandomly(n) {
    for (let i = 0; i < n; i++) {
        let node = new Node(i, Math.floor(Math.random() * WIDTH), Math.floor(Math.random() * HEIGHT));
        nodes.push(node);
        nodes2.push(node);
    }
}
